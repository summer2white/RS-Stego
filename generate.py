...
def main():
    ...
    method_path= os.path.join(outpath,"train","stego","SM")
    os.makedirs(method_path,exist_ok=True)
    method_path= os.path.join(outpath,"train","stego","D-RM")
    os.makedirs(method_path,exist_ok=True)
    method_path= os.path.join(outpath,"train","cover")
    os.makedirs(method_path,exist_ok=True)
    
    method_path= os.path.join(outpath,"test","stego","SM")
    os.makedirs(method_path,exist_ok=True)
    method_path= os.path.join(outpath,"test","stego","D-RM")
    os.makedirs(method_path,exist_ok=True)
    method_path= os.path.join(outpath,"test","cover")
    os.makedirs(method_path,exist_ok=True)
        ...
        for i, t in enumerate(iterator):
            ...
            if change_attn==True:        
                if i==len(time_range)-1:
                    ...
                    #inject=torch.randn(1,4096,2).to(device)
                    
                    #change77max=torch.load("secret/change77max.pt")
                    
                    change=inject#torch.zeros(2,4096,1).to(device)
                    # for j in range(4096):
                    #     for q in range(2):
                    #         if inject[q,j,0]*change77max[q,j,0]>0:
                    #             change[q,j,0]=change77max[q,j,0]*10     
                    #         else:
                    #             change[q,j,0]=-change77max[q,j,0]*10 
                   
                    
                    zeros4=torch.zeros(8,4096,1).to(device)
                    #zeros71=torch.zeros(1,4096,71).to(device)
                    change_16=torch.cat((zeros4,change,change,change,change),0)
                    
                    # change_16=torch.randn(16,4096,77).to(device)
                    
                    # zeros=torch.zeros(15,4096,77).to(device)
                    # change_16=torch.cat((change_16,zeros),0)
                    #change_16=inject
                    #change_16=torch.cat((inject,inject,inject,inject,inject,inject,inject,inject),0)
    ...
    train_size=100
    prompt=[ "a photo of forest fires",
              "A photo of a lion",
              "a photo of the living room" ,
              "a photo of birthday cake",
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for x_sample in x_samples_ddim:
                        x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                        img = Image.fromarray(x_sample.astype(np.uint8))
                        img.save(os.path.join(test_path,"cover",f"{nn}.png"))
                        sample_idx += 1
                    
                    # path=os.path.join(train_path,"cover",f"{nn}.png")  
                    ...
                    if add_secret_latent==True:
                        ...
                        for x_sample in x_image_torch:
                            ...
                            img.save(os.path.join(test_path, "stego",f"{method[sample_idx]}",f"{nn}.png"))
                            sample_idx += 1
