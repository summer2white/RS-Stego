...
class CrossAttention(nn.Module):
    ...
    def forward(self,
        ...
        if q_injected is None:
            ...
            if type(q_injected) is not torch.Tensor:
                q = self.to_q(x)
                q = rearrange(q, 'b n (h d) -> (b h) n d', h=h)
            else:
        ...
        # if (k_injected is not None) and (q_injected is None):
        #     print('add noise')
        #     print('atten map size=',sim.shape)
        #     print('q.shape=',q.shape,'k.shape=',k.shape)   
        #     #attn=attn+k_injected
        #     sim=sim+k_injected
                    
        attn = sim.softmax(dim=-1)
        self.attn_ori=attn.clone()
        if (k_injected is not None) and (q_injected is None):
            print('add noise')
            print('atten map size=',sim.shape)
            print('q.shape=',q.shape,'k.shape=',k.shape)   
        ...
        if (k_injected is not None) and (q_injected is None):
            print('out.shape=',out.shape,'to_out.shape=',self.to_out(out).shape,)
        return self.to_out(out)
...
class BasicTransformerBlock(nn.Module):
    ...
    def _forward(self,
                 ...
                 self_attn_q_injected=None,
                 self_attn_k_injected=None):
        #self.in_layers_features=x
        self.context=context
        x = self.attn1(self.norm1(x),
                       q_injected=None,
                       k_injected=None) + x
        #print('basic attn1 out shape=',x.shape)
        self.in_layers_features=x
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self,
        ...
        if (self_attn_k_injected is not None) and (self_attn_q_injected is None):
            print('basic_output_size=',x.shape)
