    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                torch.save(samples_ddim,'secret/samples_ddim.pt')
                ...
                cover_org=255*x_image_torch.permute(0,2,3,1).cpu().numpy()
                ...
                if add_secret_latent==True:
                    secret=np.random.randint(0,2,(n,64,64))
                    secret=torch.from_numpy(secret).to(device)
                    torch.save(secret,'secret/secret_latent.pt')
                    inject_direct=0.5*(secret-0.5).to(device)
                    
                    secret_attn=secret.reshape(n,4096,1)
                    torch.save(secret_attn,'secret/secret_attn.pt')
                    inject_attn=((secret_attn-0.5)*2).to(device)
                    q=2000
                    inject_std=((secret-0.5)*2*q).to(device) 
                    if n==1:
                        change_attn=inject_attn
                        torch.save(change_attn,"secret/inject_attn_quality.pt")
                    ...
                    samples_ddim_1=samples_ddim+change_direct
                    
                    mean=distribution.mean
                    std=distribution.std
                                       
                    samples_ddim_2 = 0.18215*(mean + std *change_std)

                    x_1=intermediates['x_1'][-1]
                    #torch.save(x_1,"secret/x_1_start.pt")
                    samples_ddim_3=sampler.hiding_cross_quality(atten_map,x_1,change_attn,device,unconditional_guidance_scale=exp_config.config.scale)
                    #torch.save(samples_ddim_3,"secret/x_64_attn_start.pt")
                    x_sample_64=torch.cat((samples_ddim_1,samples_ddim_2,samples_ddim_3),0)
                    x_samples_ddim = model.decode_first_stage(x_sample_64)
                    ...
                    for x_sample in x_image_torch:
                        ...
                        x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                        img = Image.fromarray(x_sample.astype(np.uint8))
                        img.save(os.path.join(sample_path, f"steg_{n}_{sample_idx}.png"))
                        sample_idx += 1
                ...
                for stego_uint8 in stego:
                    ...
                    print(stego_uint8.shape,cover_org[None,...].shape)
                    print(f'Quality metrics at resolution: 512x512 (HxW)')
